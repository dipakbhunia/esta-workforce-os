...
import type { AmendmentPayload, OverLimitOverride, PaginatedSubscriptions, Subscription, SubscriptionActivationSource, SubscriptionPayload, SubscriptionStatus } from './subscription.types';
...
export const runSubscriptionAction = (id: string, action: 'activate' | 'suspend' | 'resume' | 'cancel' | 'expire', payload?: OverLimitOverride) => http.post<Subscription>(`/subscriptions/${id}/${action}`, action === 'activate' ? payload ?? {} : {});
