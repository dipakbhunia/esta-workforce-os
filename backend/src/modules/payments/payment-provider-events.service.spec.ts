...
import { PaymentProviderEventStatus, PaymentProviderMode, PaymentProviderOrderStatus, PaymentProviderType, PaymentPurpose, PaymentStatus, Prisma, SubscriptionStatus } from '@prisma/client';
...
function harness(status = PaymentStatus.PENDING, truth = 'PAYMENT_CAPTURED', subscriptionStatus = SubscriptionStatus.PENDING) {
  ...
  const service = new PaymentProviderEventsService(client as never, {} as never, {} as never,
    { activate: async (id: string) => { activations.push(id); } } as never,
    { apply: async (id: string) => { renewals.push(id); } } as never);
  return { service, event, payment, order, audits, activations, renewals };
}
...
describe('PaymentProviderEventsService truth processing', () => {
  ...
  it('routes verified captured renewal truth only to renewal application', async () => {
    const h = harness(); h.payment.purpose = PaymentPurpose.SUBSCRIPTION_RENEWAL;
    await h.service.process(ids.event);
    assert.deepEqual(h.activations, []); assert.deepEqual(h.renewals, [ids.payment]);
  });
...
describe('PaymentProviderEventsService recovery selection', () => {
  it('selects bounded eligible work and isolates one poisoned event from the rest of the batch', async () => {
    ...
    const service = new PaymentProviderEventsService(prisma, {} as never, {} as never, { activate: async () => undefined } as never, { apply: async () => undefined } as never);
  ...
  it('allows only one claimant in an immediate-processor versus scheduler race', async () => {
    ...
    const service = new PaymentProviderEventsService(prisma, {} as never, {} as never, { activate: async () => undefined } as never, { apply: async () => undefined } as never); (service as any).applyTruth = async () => undefined;
...
function collisionHarness(kind: 'exact' | 'event-id' | 'payload-id' | 'split') {
  ...
  return { service: new PaymentProviderEventsService(store, credentials as never, registry as never, { activate: async () => undefined } as never, { apply: async () => undefined } as never), audits, row };
}
