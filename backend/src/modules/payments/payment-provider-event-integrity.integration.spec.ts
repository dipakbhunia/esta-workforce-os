...
describeDb('E1.6 PostgreSQL provider-event integrity', () => {
  ...
  it('converges two concurrent authenticated ingestions of the same event to one durable event', async () => {
    ...
    const service = new PaymentProviderEventsService(prisma as never, credentials as never, providers as never, { activate: async () => undefined } as never, { apply: async () => undefined } as never);
    try {
  ...
  it('serializes concurrent authorized/captured and failed/captured truth with CAPTURED winning', async () => {
    ...
    const service = new PaymentProviderEventsService(prisma as never, null as never, null as never, { activate: async () => undefined } as never, { apply: async () => undefined } as never);
    try {
